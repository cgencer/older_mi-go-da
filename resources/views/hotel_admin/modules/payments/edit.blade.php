...
@section('title')
{{trans('txt.payment_edit_reservation_edit')}} (ID: {{$data->id}}) | @parent
@endsection
@section('page_styles')
    ...
    <div class="main-content">
        <div class="container-fluid">
            <div class="page-header">
                <h2 class="header-title">{{trans('txt.payment_edit_reservation_edit')}} (ID: {{$data->id}})</h2>
                <div class="header-sub-title">
                    <nav class="breadcrumb breadcrumb-dash">
                        <a href="{{route('admin.dashboard')}}" class="breadcrumb-item"><i class="ti-home p-r-5"></i>{{trans('txt.link_home')}}</a>
                        <span class="breadcrumb-item">{{trans('txt.link_finance_management')}}</span>
                        <a class="breadcrumb-item" href="{{route('admin.reservations.index')}}">{{trans('txt.reservations')}}</a>
                        <span class="breadcrumb-item active">{{trans('txt.payment_edit_reservation_edit')}} (ID: {{$data->id}})</span>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body">
                    {{ Form::open(array('url' => route('admin.reservations.save',['id'=>$data->id]), 'method'=>'post', 'files' => e,'class'=>'m-t-15', 'autocomplete'=>'off')) }}
